...
double rosenbluth_growth::subsection_probability(std::vector<int> &limit) {
	//int l{ std::abs(limit[1] - limit[0]) + 1 };
	int l{ std::abs(limit[1] - limit[0])+1};
...
double weeks_chandler_ij(std::vector<double> position_i, std::vector<double> position_j) {// this is actually the 
	...
	if (!position_i.empty() && !position_j.empty()) {
		...
		if (r_ij <= a * std::pow(2, 1.0 / 6.0)) {
			//if (4 * (std::pow(a / r_ij, 12) - std::pow(a / r_ij, 6)) + 1 > 1000) {
			//	std::cout << "stop" << std::endl;
			//}

			return 4 * epsilon_WCA* (std::pow(a / r_ij, 12) - std::pow(a / r_ij, 6)) + epsilon_WCA;
		}
		else if (r_ij > a * std::pow(2, 1.0 / 6.0)) {
			return 0;
		}
...
double partition_ni_unlinked(int length, int NMC, double eps)
{
	set_WCA_parameter(eps);

	int i{ 0 };
	std::vector<double> old_position{ 0,0,0 };
	std::vector<double> jump(3);
	std::vector<double> weights(length-2);
	std::vector<double> energies(length-2);
	std::vector<double> generated_position(3);

	std::vector <std::vector<double>> excluded_volume;

	std::vector<std::vector<double>> trial_positions(trials), positions(length);
	std::vector<double> trial_position(3);
	std::vector<double> NMC_weights(NMC);

	positions[0] = old_position;
	//weights[0] = 1;
	//energies[0] = 0;
	//weights[1] = 1;
	//energies[1] = 0;
	...
	while (i < NMC) {
		...
		for (int m{ 0 }; m < length-1; m++) {
			if (m == 0) {
				...
				generated_position = vector_addition(old_position, jump);
				old_position = generated_position;
				positions[m + 1] = generated_position;
			}
			else if (m >0) {
				excluded_volume.push_back(positions[m-1]);

				for (int j{ 0 }; j < trials; j++) {
					sample_jump_direction(jump, 1);
					trial_positions[j] = vector_addition(old_position, jump);
				}
				generated_position = select_trial(trial_positions, excluded_volume, weights[m - 1], energies[m - 1]);
				old_position = generated_position;
		...
		//std::cout << dist_2_points3d(positions[0], positions[2]) << std::endl;;

		NMC_weights[i] = rosenbluth_factor(weights, trials);

		i++;
		std::cout << i << std::endl;
	}
	return sum_of_elements(NMC_weights)/static_cast<double>(NMC);
	...
	std::vector<std::vector<double>>& excluded_volume, double& weight, double& energy, int* selection_index)
{
	...
	for (int j{ 0 }; j < trial_positions.size(); j++) {
		...
		summand = summand +  exp(-u_r(trial_positions[j], excluded_volume));
...
void rosenbluth_sample_helix_vectors(int n, std::vector<double> &origin, std::vector<double>& u, std::vector<double>& v, std::vector<std::vector<double>>& excluded_volume_positions, double &rosenbluth_factor, std::vector<std::vector<double>> existing_helix)
{
	...
	if (excluded_volume_positions.size() == 0) {
		...
		if (forward_move == false) {
			rosenbluth_factor = cumulative_sum_of_weights / (k+1);
		}
		else {
...
std::vector<double> rosenbluth_sample_translate(std::vector<std::vector<double>>& old_helix_positions, std::vector<std::vector<double>>& excluded_volume, double translation_distance, bool forward_move, double &rosenbluth_weight)
{
	...
	for (int i = 0; i < k; i++) {
		...
		for (int j{ 0 }; j < old_helix_positions.size();j++){
			trial_helix_positions[j] = vector_addition(old_helix_positions[j], trial_translation);
			trial_energy += u_r(trial_helix_positions[j], excluded_volume);
		}
	...
	if (forward_move == true) {
		...
		rosenbluth_weight = cumulative_weight / k;
		return translations[selection];
		
	}
	else {
		rosenbluth_weight = cumulative_weight / (k + 1);
		return { 0,0,0 };
	}
}
...
std::vector<std::vector<double>> generate_corkscrew(helix_struct* dh, std::vector<std::vector<double>> helix_positions, double angle)
{
	...
	int gamma{ static_cast<int>(std::pow(-1,beta)) };
	double pitch{ 0.28 };
...
void rosenbluth_sample_spin(helix_struct* dh, std::vector<std::vector<double>>& old_helix_positions, std::vector<std::vector<double>>& excluded_volume, bool forward_move, double& rosenbluth_weight, double& selected_angle, std::vector<double>& selected_rot_axis)
{
	...
	if (excluded_volume.size() == 0) {
		generate_spin(dh, old_helix_positions, selected_angle, selected_rot_axis);
		rosenbluth_weight = 1.0;
		return;
	}
