...
std::vector<double> generate_log_array(double start, double end, int numElements) {
    if (start <= 0 || end <= 0 || numElements < 2) {
    ...
    double logStep = std::log10(end / start) / (numElements - 1);

    // Generate the logarithmically spaced array
    for (int i = 0; i < numElements; ++i) {
        double value = start * std::pow(10, logStep * i);
