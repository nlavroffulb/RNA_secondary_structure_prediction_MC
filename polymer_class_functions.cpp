    ...
    for (int i{ 0 }; i < new_config_positions.size(); i++) {
        ...
        if (j + 1 > new_config_positions.size() - 1) {
            ...
            if (dist_2_points3d(temp1, temp2) > 5) {
                ...
                double_vector_to_txt("caught_one.txt", {});
...
{
    //if (helix_R_factor != 1.0) {
    //    std::cout << "stop" << std::endl;
    //}
    return helix_R_factor;
}

void polymer::always_accept_link_move(bool& overstretch_reject)
{
...
void polymer::KH_unlink_move(bool& overstretch_reject, std::vector<std::vector<int>> unlink_prohibited)
{
    overstretch_reject = false;
    ...
    for (auto i : unlink_prohibited) {
        if (i == helix_list[s_index]->get_monomers()) {
            return;
        }
    }
